...
"""
train_job_tmpl = """
srun -n 1 --output="%s" $CAFFE train -gpu %d --solver=%s &
"""
train_job_tmpl_finetune = """
srun -n 1 --output="%s" $CAFFE train -gpu %d --solver=%s --weights=%s &
"""
train_job_tmpl_resume = """
#srun -n 1 --output="%s" $CAFFE train -gpu %d --solver=%s --snapshot=%s &
"""
train_footer_tmpl = """
wait
"""
...
for job in jobs:
  ...
  # make new network 
  train_file = job_path + 'train_test.net'
  ...
  log_file = jobs_root + job['name'] + '.log'
  job_files.append([solver_file, model_file, log_file, snapshot_file])
  ...
  for (i,batch) in enumerate(batcher(job_files,2)):
    ...
    with open(sbatch_file, 'w') as f:
      ...
      for (igpu, files) in enumerate(batch):
        solver, model, log, snapshot = files
        if model:
          f.write(train_job_tmpl_finetune % (log, igpu, solver, model))
        else:
          f.write(train_job_tmpl % (log, igpu, solver))
        f.write(train_job_tmpl_resume % (log, igpu, solver, snapshot))
