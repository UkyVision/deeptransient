...
In this section, we show how to geolocalize an outdoor webcam using
TransientNet. The intuition is that, given a sufficient amount of time, the
temporal pattern of the transient attributes is a unique fingerprint of the
location. We adopt the framework of~\cite{jacobs07geolocate}, in which the
webcam location is found by relating pixel variations of geo-registered
satellite imagery and the PCA coefficients of webcam images. The only change is
replacing the PCA coefficents (which are unsupervised, but camera specific)
with the transient attributes (which are supervised, but not camera specific).
...
images\footnote{\url{http://mesonet.agron.iastate.edu}}.  We tried all
transient attributes and found that the \textit{sunny} attribute provided the
most accurate localization results, we use it for all further figures.
\figref{geoloc} shows examples of the heat-maps we estimate using our method.
\figref{locerror} shows the evaluation results.  Using \textit{sunny}
attribute, we managed to locate 58\% of webcams within 250km from the ground
truth. As a baseline method, we repeated the experiment with the top 5 PCA
coefficients. The best coefficient (the first) only locates 9\% of webcams
within 250km error. We think the main advantage of using the transient
