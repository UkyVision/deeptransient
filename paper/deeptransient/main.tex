...
\vspace{-1em}
\paragraph{Network Architectures and Training:} For each network, we start the training procedure from three different initial conditions,
...
most accurately. Our method is also very fast; it averages 0.0672s per
image for prediction. 
\todo{do we have any performance numbers for their method?  if not,
perhaps we drop this and only say the numbers for the next experiment}

\subsection{Transient Attributes Estimation}

We evaluted our three TransientNet variants on the dataset created by
Laffont et al.~\cite{Laffont14}. \figref{compare} shows the average
...
using their transient attributes (\figref{geoloc}). Since the local
weathers vary from place to place, the changing pattern of weather
attribute can be treat as a figure print in that region.

Using the framework of~\cite{jacobs07geolocate}, where the webcam
location is found by computing the correlation map between pixel
densities of geo-registered satellite imagery and the PCA coefficients
of webcam frames, we replace the webcam PCA coefficents with transient
attribute features.
...
We select 180 webcams for evaluation (\figref{webcamdist}). The satellite imagery used
in our experiment can be downloaded from~\footnote{mesonet.agron.iastate.edu}.
