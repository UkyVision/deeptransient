...
\usepackage{subfigure,multirow,bigstrut,paralist}
...
AMOS~\cite{jacobs07amos} webcams across a subset of the transient attributes.
The 100 images with the highest score for each attribute (per the \textit{fc8}
output layer) were averaged to create each of the images in \figref{netvis}.
The average images for most of the attributes in the visualization are a
reasonable representation of the attribute for the scene.  The seasonal
attributes (\textit{autumn, summer, etc.}) show how the scene changes
throughout the year and lighting attributes (\textit{sunrise/sunset, daylight,
night, etc.}) shows the scene in various lighting conditions.  Some attributes
have similar average images though (\textit{e.g., glowing and colorful, windy
and rain}), suggesting that the network may not distinguish between these
attributes well or that these attributes have some correlation with each other.
...
%In future work, we plan to explore the use of these networks in an application
%to micro-climate estimation from publicly available outdoor
%webcams~\cite{islam13webcamweather}. 
