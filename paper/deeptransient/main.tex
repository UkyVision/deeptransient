...
geolocalization~\cite{jacobs07geolocate,workman2015localize}; and
environmental monitoring~\cite{jacobs09webcamgis}. 
...
We convert \emph{AlexNet} into CloudyNet by modifying the network architecture;
we update the final fully connected layer to have two output nodes.
...
different times of the day and on different days of the year.  The final dataset
...
\vspace{-1em}
\paragraph{Network Training:} For each network architecture, we start the
training procedure from three different initial conditions, resulting in six
distinct sets of network weights. The first set of initial conditions were
taken from a network that was was trained for object classification on 1.2
million images with 1000 object class from the ImageNet ILSVRC-2012
...
containing about 3.6 million images.  We call the resulting networks
CloudyNet-H and TransientNet-H.
...
%images.
\tblref{twoclass} compares the mean normalized accuracy and variance for our
networks against the previous best technique.  The normalized accuracy, which is
...
\subsection{Transient Attribute Estimation}
...
a single image, we found Laffont et al.'s method takes an average of 3.486
seconds, but our method only requires 0.192 seconds, an 18x speed up. 
...
attributes (\textit{sunrise/sunset, daylight, night}) show the scene in
...
images using CloudyNet-H:  an overcast scene of a mansion that was
classified as sunny and a clear scene of a country home that was classified
as cloudy.

\begin{figure}[t]
	\centering
		\includegraphics[width=\linewidth, trim= 1mm 0mm 1mm 0mm]{figs/montage_pruned_cam_7211.pdf}
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.49\columnwidth}
    \centering
		\includegraphics[width=\columnwidth]{figs/ex_bad_tc/cloudy.jpg}
    \caption{Misclassified as \emph{sunny}}
    \label{fig:ex_bad_cloudy}
	\end{subfigure}	
  \begin{subfigure}[b]{0.49\columnwidth}
    \centering
		\includegraphics[width=\columnwidth, trim= 0mm 24mm 0mm 0mm, clip]{figs/ex_bad_tc/sunny.jpg}
    \caption{Misclassified as \emph{cloudy}}
...
\begin{figure}
  ...
  \caption{Composite images generated using the fully convolutional TransientNet-H.
           Brighter areas in each of the color channels indicate a higher
           attribute value.}
...
\begin{figure}
  ...
  \begin{subfigure}[b]{\columnwidth}
    ...
    \label{fig:snow}
	\end{subfigure}	
	\caption{Example attribute summaries over a year of webcam data (green=attribute
           present, gray=uncertain, red=attribute absent, white=no data).  The highlighted
...
channels.  There are no snowy areas in the input image, shown in the blue
channel, and the bottom of the image contains high values for the lush
attribute, shown in the green channel.  The sunny attribute is higher towards
the horizon and middle of the sky, shown in the red channel, possibly due to
the sky being brighter in these regions.  \figref{false_color_2} shows a
composite image using the \textit{sunny}, \textit{storm}, and \textit{snow}
...
show in the green channel.  The storm attribute is higher in the overcast sky
towards the top of the composite image.  The snow covered ground appears in the blue
channel with high values for the snow attribute around the middle of the image
and a dark spot corresponding to the waterway in the scene.
...
webcams with certain attributes.  If an attribute is above a threshold (e.g., $t_h =
0.75$), the image is labeled with that attribute.  The opposite is true as
well. If an attribute is below a threshold (e.g., $t_l = 0.25$), the attribute is
added to a list of attributes the image does not have. This enables users to
find, for example, images that are both snowy and sunny using queries such as
...
\begin{figure*}
  ...
  \end{subfigure}
  \caption{Maps of the \emph{snow} attribute from webcam data (bottom) across the continental United 
           States in January 2014 and the
...
\begin{figure}[th!]
	\centering
		...
		\caption{Map of the \emph{snow} attribute from \figref{snow_map_est_3} with three highlighted snowy 
             images.}
		\label{fig:snow_exs}
\end{figure}
...
during the summer months. The \textit{night} and
\textit{daylight} attributes clearly show the day/night cycle for the location
of the image.  Properties about the scene can be
...
technique proposed by Murdock et al.~\cite{murdock13clouds, murdock2015cloudmap}
...
transient attributes is a unique fingerprint of a location. Based on
this observation, we propose a robust method for geolocalizing outdoor
...
\begin{figure}[t]
  \centering
  \includegraphics[width=.98\linewidth, trim= 14mm 3mm 15mm 0mm]{figs/geoloc/tran_errors}
  \includegraphics[width=.98\linewidth, trim= 14mm 3mm 15mm 0mm]{figs/geoloc/pca_errors}
...
visualizes geolocalization results for several webcams and \figref{locerror}
shows quantitative results. Our method localizes 58\% of webcams within 250km
from the ground truth. As a baseline method, we repeated the experiment with
the top 5 PCA coefficients. The best coefficient (the first) only locates 14\%
of webcams within 250km. We think the main advantage of using the transient
attribute for this task is that it is less sensitive to camera jitter, a
significant problem when applying PCA to outdoor webcam data. When the camera
jitters it is likely that the PCA coefficients encode for motion, not changes
...
two benchmark datasets, requires no hand-engineered features, is
