...
attribute's value across an input image, as shown in \figref{false_color_ims}.
The values for each attribute can be visualized in a single channel image.
Combining three of these images results in the composite images.
\figref{false_color_1} shows a composite image using the \textit{sunny},
\textit{lush}, and \textit{snow} attributes as the color channels.  As
expected, there are no snowy areas in the input image, shown in the blue
...
\footnotetext{\url{http://www.nohrsc.noaa.gov/nsa/index.html}}
\begin{figure}[t]
	\centering
